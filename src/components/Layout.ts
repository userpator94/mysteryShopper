  ...
  layout.innerHTML = `
    ...
    <footer id="main-footer" class="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-sm border-t border-slate-200">
      <div class="flex justify-around items-start pt-2 pb-3">
        <!-- Навигация для исполнителя (user) -->
        <div id="nav-user" class="flex justify-around items-start w-full" data-nav-role="user">
        ...
        <div id="nav-employer" class="hidden flex justify-around items-start w-full" data-nav-role="employer">
          <a class="flex flex-col items-center justify-center gap-1 text-slate-500 w-1/4 nav-link" href="#" data-route="/">
            ...
            <p class="text-xs font-medium">Мои задачи</p>
          </a>
          <a class="flex flex-col items-center justify-center gap-1 text-slate-500 w-1/4 nav-link" href="#" data-route="/my-offers/new">
            <svg fill="currentColor" height="24" viewBox="0 0 256 256" width="24" xmlns="http://www.w3.org/2000/svg">
...
function updateActiveNavLink(activeRoute: string) {
  ...
  navLinks.forEach(link => {
    ...
    if (isActive) {
      ...
      if (icon) { icon.classList.remove('text-slate-500'); icon.classList.add('text-primary'); }
    } else {
      link.classList.remove('text-primary');
      link.classList.add('text-slate-500');
      if (icon) { icon.classList.remove('text-primary'); icon.classList.add('text-slate-500'); }
    }
  });
}
...
export function updateNavByRole(): void {
  ...
  const role = getRole();
  if (navUser) navUser.classList.toggle('hidden', role === 'employer');
  if (navEmployer) navEmployer.classList.toggle('hidden', role !== 'employer');
  const path = window.location.pathname || (window.location.hash ? window.location.hash.slice(1).split('?')[0] : '/');
